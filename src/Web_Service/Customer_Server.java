...
public class Customer_Server extends HttpServlet {
    ...
    public Customer_Server() {
        ...
        try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/jit_dispatcher","root","root");
		} catch (Exception e) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("tech_id")==null) {
			...
			System.out.println("Cust ID: " + request.getParameter("cust_id"));
			System.out.println("Type: " + request.getParameter("product_id"));
			...
			System.out.println("Tech ID: "+ request.getParameter("tech_id"));
			JSONObject tech = new JSONObject();
			java.sql.Timestamp now = new Timestamp(new java.util.Date().getTime());
			try {
				st = con.createStatement();
				query ="update tech set status='commited' where tech_id ="+tech_id;
				//st.executeUpdate(query);
				
				query ="insert into service(cust_id, tech_id, prod_id, status, request_time) values"
			...
			String res = "confirmTechMessage" + "(" + tech + ")";
